...
app.layout = html.Div([
    ...
    html.Hr(style={'borderTop': '2px solid white', 'width': '50%', 'margin': '0 auto', 'backgroundColor': 'white', 'border': 'none', 'height': '2px'}),
    html.Br(),
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                dcc.Dropdown(id='visualization-dropdown',
                             options=[
                                 ...
                                 {'label': 'Heatmap of Trips per Hour and Day of the Week', 'value': 'hourly-weekly-heatmap'},
                ...
                # Add the legend right after the visualization graph
            ], style={'width': '100%', 'height': '100px', 'margin': '0 auto'}),
        ], width=6),
        dbc.Col([
            html.Label('Select a blue bike station:', style={'color': 'white'}),
            dcc.Dropdown(id='start-station-dropdown', options=[{'label': i, 'value': i} for i in sorted(df_data['start station name'].unique())],
...
def update_visualization_message(selected_visualization):
    if selected_visualization == 'monthly-trend':
        message = "The Monthly Trend Analysis visualization shows the number of trips per month in 2020. The monthly trend data reveals a seasonal pattern in bike-sharing usage, with higher numbers of trips occurring during the warmer months and lower numbers during the colder months. A significant dip in April 2020 is likely due to the impact of COVID-19 lockdowns. The peak usage occurs in August and September 2020, which can be attributed to favorable weather conditions. Overall, bike-sharing usage is heavily influenced by seasonal weather patterns"
    elif selected_visualization == 'trip-duration':
        message = "The Trip Duration Analysis visualization shows the distribution of trip durations in seconds (0-2000). The trip duration data shows that the distribution of bike trips is right skewed, with most trips being of shorter durations. The highest number of trips fall within the 300-349 seconds range, followed by a gradual decline in frequency. The majority of users seem to prefer shorter trips, possibly for commuting or leisure purposes, indicating that bike-sharing is primarily utilized for shorter, more convenient journeys."
    elif selected_visualization == 'customer-subscriber':
        message = "The Customer vs Subscriber Analysis visualization shows the proportion of trips made by customers and subscribers. The data from the customer vs. subscriber analysis indicates that a significant majority (72%) of bike-sharing users are subscribers, while the remaining 28% are customers. This suggests that bike-sharing services are popular among regular users who find value in subscribing to the service, possibly for daily commutes or frequent short trips."
    elif selected_visualization == 'hourly-weekly-heatmap':
        message = "The Heatmap of Trips per Hour and Day of the Week shows the number of trips per hour and day of the week. Weekdays experience the highest number of trips during morning (7-9 AM) and evening (5-7 PM) commute hours, suggesting that the service is primarily used for work commutes. On weekends, there is slightly higher usage during the daytime and evenings, likely due to recreational and social activities. Early mornings (0-4) across all days witness the lowest number of trips, with a sharp decline in trips after 10 PM."
    elif selected_visualization == 'top-stations':
        message = "The Top Start and End Stations visualization shows the top 10 start and end stations by number of trips. The data highlights the top 10 start and end stations, with Central Square at Mass Ave / Essex St and Charles Circle - Charles St at Cambridge St being the most popular. Other significant stations, such as MIT at Mass Ave/Amherst St and Christian Science Plaza, are also popular. Some stations, like Cross St at Hanover St, are more popular start stations, and others, like Nashua Street at Red Auerbach Way, are popular end stations."
    elif selected_visualization == 'postal-code':
        message = "The Postal Code Analysis visualization shows the top 10 postal codes by the number of trips. The postal code analysis shows that the highest number of trips are concentrated in the 02139 postal code (Cambridge, MA), with more than double the trips compared to the second highest, 02215. Given that the highest concentration is in 02139, it suggests that the Cambridge area is a crucial region for the service. This could be due to the presence of key locations, such as educational institutions, businesses, or popular destinations."
    else:
        message = ""
...
def update_map(selected_start_station, start_date, end_date):
    ...
    # Save map to HTML string and return it
